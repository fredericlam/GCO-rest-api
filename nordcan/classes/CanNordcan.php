	...
	{	
		...
		*/ 
		...
		{	
			...
			$year = $settings['year'] ; 
			$table = "nordcan_predictions" ; 
			$table_pop = "nordcan_country" ; 
		...
		*/
		public static function getNumbers( $settings ){
			...
			if ( $settings['type'] == 2 )
			{
				$table = "nordcan_prevalence" ; 
				$query = " SELECT t1.*,t3.ICD,t3.label as cancer_label FROM $table t1 " ; 

				// left join to get information on population (continent, area, hdi , hub ... )
				$query .= "LEFT JOIN nordcan_country t2 ON t2.country = t1.country " ; 
				$query .= "LEFT JOIN nordcan_cancer t3 ON t3.cancer = t1.cancer " ; 
			...
			if ( !empty( $settings['years'] ) && !empty($settings['years'][0]) ) 
			{
				if ( $settings['years'][0] != '' ) 
					$query .= "year IN ".self::imp($settings['years'])." AND " ; 
			}
			else if ( !empty( $settings['year_start'] ) && !empty( $settings['year_end'] ) ) 
			{
				$query .= "year BETWEEN ".(int)($settings['year_start'])." AND ".(int)($settings['year_end'])." AND " ; 
			}
			...
			if ( $settings['type'] == 2 )
				...
				$query .= " ORDER BY year, {$settings['sort']} {$settings['sort_dir']} " ; 
			...
			while ( $data_row = $execute->fetch() )
			{
				...
				if( $settings['sub_mode']=='cancer' ) 
					...
					$label = $settings['items'][(int)$data_row['country']]['label'] ;
				}
			...
			if ( $settings['grouped'] == 0 ) // if there's no grouping, then get all per population/countries and check if there is a grouping per cabcer
			{
		...
		* @return (array)
		*/
		public static function numbersGroupBy( $data , $key , $sub_mode , $items = [] , $sex )
		{	
			...
			foreach( $data as $index => $row )
			{
				...
				if ( $sub_mode == 'population')
				{
					...
					$final[ $key_row ]['ages'][(int)$row['cancer']]	= $row['ages'] ; 
			...
			switch ( $sub_mode )
			{
				...
				case "population" :  
					...
					foreach( $final as $key => $row )
					{
						...
						// debug($row); debug($cases_ages); debug( $total_population) ; exit() ; 

						// calculate new values, with cumulated population
						$asr 		= getASRCancer( $cases_ages, $populations ) ; 
						$crude_rate = getCrudeRate( array_sum($cases_ages) , $total_population ) ;

						// var_dump($crude_rate); exit() ; 
						...
						// get age specific rate
						foreach( $final[ $key_population ]['cancers'] as $y => $row_cancer )
						{
							$age_specific_rate = [] ; 
							$cpt = 0 ; 
							foreach( $row_cancer['ages'] as $key_age => $age )
							{
								$age_specific_rate[$key_age] = getCrudeRate( $age , $populations[$cpt] ) ; 
								$cpt++ ; 
							}

							$final[ $key_population ]['cancers'][$y]['age_specific_rate'] = $age_specific_rate ; 
						}
		...
		*/
		...
		public static function checkGroupingCancers( $final , $params_settings = NULL )
		{
			...
			{				
				...
				$sub_settings['items'] = CanNordcan::getPopulations( 'country' , false , [] , 'country DESC' , $sub_settings ) ; 
                $sub_settings['sub_mode'] = 'population' ;                 
                $output = CanNordcan::getNumbers( $sub_settings ) ; 
		...
		*/
		...
		public static function getRealGroupingNumbers( $settings )
		{
            ...
            // $sub_settings['items']          = CanNordcan::getCancers(true,false,$sub_settings) ;  
			// var_dump($sub_settings);exit(); 

            $output = CanNordcan::getNumbers( $sub_settings ) ; 
            $rows = CanNordcan::optimizeNumbers( $output );
				...
				if ( $settings['grouped'] == false )
				{
					...
					foreach( $settings['cancers'] as $cancer )
					{
						...
						$sub_settings['sub_mode'] = 'cancer' ; 
 						$rows_cancers = CanNordcan::getNumbers( $sub_settings ) ; 
		...
		*/
		...
		{		
			...
			foreach( $groups as $k => $group )
			{
				...
				$ids = CanNordcan::getCountriesPerGroup( $key , $group['id']  ); 
	            ...
	            $rows = CanNordcan::getNumbers( $sub_settings ) ; 
		...
		*/
		...
		public static function runCohort( $input )
		{
			...
			foreach( $conf['y'][ $settings['year'] ]['ages'] as $key_age => $age ) 
			{
				$Values 		= [] ; 
				$base_age 		= @explode('_',str_replace('N','',$age)) ; 
				$end_year_age 	= ( isset( $base_age[1] ) ) ? (int)$base_age[1] : (int)$base_age[0] ; 
				$end_year_age 	= $end_year_age + 2.5 ; 

				foreach( $conf['y'][ $settings['year'] ]['ages'] as $step_age => $age_analyse ) 
				{
					// $field = $age ; 
					$tmp_range = explode('_',str_replace('N','',$age_analyse)) ; 

					if ( isset( $tmp_range[0]) && isset( $tmp_range[1]) )
					{
					 	$k_start = $tmp_range[0] ; 
					 	$k_end = $tmp_range[1] ; 

						$End_year 	= ($y_start-$k_start) ;
						$Start_year = ($y_start-$k_end) ; 

						// create specific row period for age band 
						$row_range_age = [ 
							'range' 		=> "$Start_year-$End_year" , 
							'year_cohort' 	=> ($End_year - $end_year_age) , 
							'year_period' 	=> $End_year ,  
							'value' 		=> 0 
						] ; 
						...
						foreach( $input as $key => $row )
						{
							if ( $row['year'] >= $Start_year && $row['year'] <= $End_year )
							{
								...
								$total_val += $row['cancers'][ $settings[ 'cancers' ][0] ]['ages'][ $age ] ; 
								$total_pop += $row['populations'][$key_age] ; 

								$total_val_a[] = $row['cancers'][ $settings[ 'cancers' ][0] ]['ages'][ $age ] ; 
								$total_pop_a[] = $row['populations'][$key_age] ; 
							}
						}	

						// echo $total_val ." ". $total_pop.'<br>' ;
						// $row_range_age['operations'] = [$total_val_a,$total_pop_a] ; $row_range_age['operation'] = [$total_val,$total_pop] ;
						
						$row_range_age['value'] = getCrudeRate($total_val,$total_pop);

						if ( $row_range_age['value'] > 0 )
						{
							// build left key (ex:2011-2015)
							$Values[] = $row_range_age ; 
						} 
					}
				}
		...
		*/
		public static function getCountriesPerGroup( $field , $value )
		{
			...
			$table = "nordcan_numbers" ; 
			$query = " SELECT country FROM nordcan_country WHERE $field = '$value' AND iso_3_code <> '' " ; 
			// echo $query . '<br>' ; 
		...
		*/
		public static function getCountriesPerGroupKeys( $field , $values )
		{
			...
			$table = "nordcan_numbers" ; 
			$query = " SELECT country FROM nordcan_country WHERE $field IN ('$value') AND iso_3_code <> '' " ; 
		...
		{	
			...
			$query .= " FROM `globocan2018_numbers` t1 , `globocan2018_cancer` t2 " ;
		...
		public static function getMaxPerCountries( $settings )
		{
			...
			$query 		= "SELECT country , label FROM `globocan2018_country` WHERE country < 900 " ;
			...
			foreach( $results as $row )
			{
				...
				$sub_query = "SELECT t1.cancer , t1.asr , t1.total , t1.crude_rate  , t1.cum_risk , t2.label , t2.color FROM `globocan2018_numbers` t1 , `globocan2018_cancer` t2 WHERE t1.country = {$row['country']} AND t1.type = {$settings['type']} AND t1.sex = {$settings['sex']} AND t1.cancer NOT IN (".implode(',',$excludes_cancers).") AND t1.cancer = t2.cancer ORDER BY {$settings['field_key']} DESC LIMIT 1 " ;
				...
				if ( (bool)$settings['grouping_cancer'] == true )
				{
					...
					$tmp = CanNordcan::getNumbers( $sub_settings ) ; 
					$row_grouped = $tmp[0] ; 
		...
		public static function getTotalCancers( $type = 0 , $sex = 0 , $population = 900 , $prevalence = false , $statistic = 0 )
		{
            ...
            if ( $prevalence == false )
                $query = " SELECT total FROM nordcan_numbers WHERE cancer = $all_cancer_id AND type = $type AND sex = $sex AND country = $population " ;
            else
                $query = " SELECT total FROM nordcan_prevalence WHERE cancer = $all_cancer_id AND survival = $statistic AND sex = $sex AND country = $population " ;
		...
		*/ 
		public static function getCancers( $with_all = false , $ages = false , $settings )
		{
			...
			$QUERY_ALL = '' ; 
			if ( $with_all == false ) $QUERY_ALL = ' cancer NOT IN ('.implode(',',$conf['y'][ $settings['year'] ]['all_cancers_id']).') ' ; 
			...
			$query .= "  WHERE LANGUAGE = {$settings['language']} AND $forbidden_cancers_query $QUERY_ALL ORDER BY label" ;
			$execute = $o_bdd->query( $query ) ; 
		...
		*/ 
		public static function getPopulations( $key = 'country' , $addStats = false , $stats = [] , $order_label = 'country DESC' , $settings = [] )
		{
			...
			$year = $settings['year'] ; 
			$populations_table = "nordcan_country" ;

			$query 		= " SELECT * FROM $populations_table WHERE LANGUAGE = {$settings['language']} ORDER BY $order_label  " ;
		...
		*/ 
		public static function getAllDataSourceMethod()
		{
			...
			$year = $settings['year'] ; 
			$populations_table = "nordcan_source_methods" ;
