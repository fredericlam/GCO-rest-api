<?php
	...
	foreach( $cancers as $cancer )
	{
		foreach( $types as $type )
		{
			...
			{
				$url = $_SERVER['HTTP_HOST']."/api/globocan/v1/2018/data/cancer/{$type}/{$sex}/{$pops_str}/{$cancer['cancer']}/" ; 
				// exit($url);
	...
	foreach( $cancers as $cancer )
	{
		foreach( $types as $type )
		{
			foreach( $sexes as $sex )
			{
				$url = $_SERVER['HTTP_HOST']."/api/globocan/v1/2018/data/cancer/{$type}/{$sex}/{$pops_str}/{$cancer['cancer']}/?ages_group=0_14" ; 
				// exit($url);
				// $content_file = file_get_contents( $url ) ; 
	...
	foreach( $cancers as $cancer )
	{
		foreach( $statistics as $statistic )
		{
			foreach( $sexes as $sex )
			{
				$url = $_SERVER['HTTP_HOST']."/api/globocan/v1/2018/data/cancer/2/{$sex}/{$pops_str}/{$cancer['cancer']}/?statistic={$statistic}" ; 
				// exit($url);
				// $content_file = file_get_contents( $url ) ; 
