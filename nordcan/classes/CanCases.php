	...
	class CanCases
	{
		...
		*/ 
		public static function getPop( $params , $one = false , $tableName = 'pop' )
		{
			...
			$query 		.= " FROM $table WHERE " ;
			$query 		.= " country = ".$params['country']. " AND year = ".$params['year']."" ; 
		...
		*/ 
		...
		public static function getCumulatedPopulations ( $params )
		{
			...
			if ( count($params['countries']) == 0 ) return [] ; 

			$query 		 = " SELECT $sum_query" ; 
			$query 		.= " FROM $table WHERE" ;
			$query 		.= " country IN (".implode(',',$params['countries']). ") AND sex= ".$params['sex']." AND year= ".$params['year'] ;
			
			// var_dump($params['countries']);
			// echo $query .'<br>' ; 
			
			$execute 	= $o_bdd->query( $query ) ; 
			$results 	= $execute->fetch(PDO::FETCH_NUM) ; 
